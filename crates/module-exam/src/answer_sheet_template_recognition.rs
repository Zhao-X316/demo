...
use crate::answer_sheet_recognition::{
    AnswerSheetAnchor, AnswerSheetItemTemplate, AnswerSheetTemplateDefinition, LocalOmrPolicy,
    ANSWER_SHEET_TEMPLATE_SCHEMA_VERSION,
};
use crate::objective_recognition::ObjectiveQuestionType;
use crate::ordinary_paper_recognition::OrdinaryPaperItemSpec;
...
impl AnswerSheetTemplateRecognitionOutput {
    pub fn validate_against(
        ...
        let expected = request
            ...
            .iter()
            .map(|item| {
                let question_type = match item.question_type {
                    crate::ordinary_paper_recognition::OrdinaryPaperQuestionType::Single => {
                        ObjectiveQuestionType::Single
                    }
                    crate::ordinary_paper_recognition::OrdinaryPaperQuestionType::Multiple => {
                        ObjectiveQuestionType::Multiple
                    }
                    crate::ordinary_paper_recognition::OrdinaryPaperQuestionType::TrueFalse => {
                        ObjectiveQuestionType::TrueFalse
                    }
                };
                (item.assessment_item_id, question_type)
            })
        ...
        let actual = self
            ...
            .collect::<BTreeMap<_, _>>();
        if actual.keys().any(|id| !expected.contains_key(id)) {
            return Err(CoreError::Invalid(
...
mod tests {
    ...
    use crate::ordinary_paper_recognition::OrdinaryPaperQuestionType;

    fn request<'a>(bytes: &'a [u8], hash: &'a str) -> AnswerSheetTemplateRecognitionRequest<'a> {
        let items = Box::leak(Box::new([OrdinaryPaperItemSpec {
