...
fn makeup_good_pass_restarts_schedule_without_erasing_lapse() {
    ...
    submissions::set_recognition(&conn, makeup_sub, Some(&answer), "ok", None, Some(5000)).unwrap();
    ...
    let scored = score_submission(&conn, makeup_sub, &words, day2, &ScoreCfg::default()).unwrap();
    assert!(scored.pass);
...
fn retention_strengthens_across_two_seven_and_twenty_one_day_windows() {
    ...
    let (conn, sid, cid, first_sub) = setup_imported(answer);
    let day1 = NaiveDate::from_ymd_opt(2026, 6, 25).unwrap();
    submissions::set_recognition(&conn, first_sub, Some(answer), "ok", None, Some(5_000)).unwrap();
    score_submission(&conn, first_sub, &[], day1, &ScoreCfg::default()).unwrap();
    human_decide(
        &conn,
        first_sub,
...
fn older_decision_cannot_change_after_newer_scope_effect() {
    ...
    let (conn, sid, cid, first_sub) = setup_imported(answer);
    let day1 = NaiveDate::from_ymd_opt(2026, 6, 25).unwrap();
    submissions::set_recognition(&conn, first_sub, Some(answer), "ok", None, Some(5000)).unwrap();
    score_submission(&conn, first_sub, &[], day1, &ScoreCfg::default()).unwrap();
    human_decide(
        &conn,
        first_sub,
    ...
    let day2 = NaiveDate::from_ymd_opt(2026, 6, 26).unwrap();
    submissions::set_recognition(&conn, second_sub, Some(answer), "ok", None, Some(5000)).unwrap();
