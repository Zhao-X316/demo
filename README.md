...
> 2026-09-07 前端简化候选：主导航为“工作台 / 资料 / 学生”，批改按“资料 → 系统处理 → 老师核对 → 结果”组织。任务从现有领域记录读取，已保存批次可重新打开；切换主导航保留当前输入，评分确认与发布继续分开。资料默认找题，统计与学习证据移入学生。详见 [模块职责](docs/architecture/module-boundaries.md) 和 [原简化方案](docs/refactor-plans/frontend-simplification-2026-09-07.md)。本次是本地实现，真实教师 R5 效果验收仍待试点。
