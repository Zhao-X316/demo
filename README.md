...
> 2026-07-15：独立分支 `codex/t2-artifacts@bc15cf2` 已完成 T2～T6、B3a0、B3a1 普通卷纵切、B3a2 固定答题卡首建/复用，以及 M2-C0 固定格式默写首条纵切。普通卷由老师确认结构后识别；稳定四锚点答题卡只需确认一次空白模板，之后整批按题号格位做透视校正与本地 OMR；固定默写只需确认一次空白页行/栏/题号区域和当前答案/rubric，之后自动生成题区裁剪、调用不含标准答案的手写 OCR，并把分歧、未写、无法辨认、识别失败和涂改歧义送到“默写复核”。老师校正追加 revision，机器原文永不覆盖。所有链均不能自动确认分数、发布或生成正式学习证据。真实照片/真实 provider、稳健透视校正、多页逐页模板、无稳定锚点答题卡、主观区、答案结构化与三类黄金集仍未完成。未 tag、合并、推送或发布。
...
| `crates/module-exam` M2 | 🟡 A1 + B0/B1 + M2.5 + B2 + B3a1/B3a2 + M2-C0 纵切 | 手工纵切已真机通过；普通卷、稳定四锚点答题卡和固定格式默写首建/复用链已落；默写机器结果仍只是建议，尚未接成绩终审/发布 |
...
| M2 傻瓜式固定卷预检 | 🟡 三材料固定版式首建与复用已贯通 | “上传批改”仍是唯一主入口；普通卷确认结构，答题卡确认一次空白模板，默写确认一次空白区域/评分策略，之后整批自动处理并只展示异常。答案自动结构化、真实照片 GUI/阈值、自由默写和无稳定锚点答题卡仍待施工 |
| 音频回放 | ✅ app-managed archive | 按 hash 归档；原文件改名、重启和数据库恢复后仍可回放，原路径只兜底 |
| ffmpeg 转码/时长探测 | ✅ 可选集成 | 装了 ffmpeg 则转 16k 单声道 wav + 探测时长，否则降级 |

当前已验证：module-exam **82 tests**、workspace **211 tests**、Tauri 应用 **41 tests**、两套 Clippy `-D warnings`、Tauri check、前端 build 和 `git diff --check` 全通过；exam 迁移已到 `exam_0017`。默写回归覆盖模板必须完整映射全部题区、裁剪 lineage、答案不进入 OCR 请求、错误年份原样保留、老师校正追加 revision、旧失败 run 保留与新幂等键重试，以及处理阶段 0 grade decision / 0 publication。真实答题卡/默写照片、真实 provider 凭据、阈值和 GUI 尚未验证；固定默写首版只支持固定页面、每项一个评分点，非 recognized 状态仍走老师补录，且尚未生成正式成绩或学习证据；正式库未写入。
