...
> 2026-07-18：M1.1 已完成结构化 rubric/评分点、ASR 与评分 run、老师逐点评审、评分点首用、疑点区间跳播、风险快捷终审、历史证据回看及隔离 `.app` G01～G05/重启核验。M1.2-1（`9f1f530`）已把老师确认的总体、流畅度和逐点评审投影为共享 `learning_evidence`；M1.2-2（`9f4b4d8`）新增不可变终审日期上下文和跨日期 retention 窗口，同日重复通过 revision 去重，补背恢复不抹去旧失败，改判与排程/effect/证据保持同事务可逆。旧验收库副本已由 53 升到 54 个迁移且 16 条既有证据不变；旧 effect 因缺可信日期不猜测回填。下一包是黄金集 runner；真实 provider、学生纵向录音、老师耗时仍未验证。未 tag、合并、推送或发布。
...
| `crates/module-recitation` M1 域 | ✅ M1.1 + M1.2-2 | 文件名解析、结构化 rubric/评分点、不可变 transcript/AI run、熟练度与评分编排、老师逐点评审、共享总体/逐点/保持学习证据 |
...
当前已验证：module-exam **182 tests**、module-knowledge **6 tests**、module-profile **23 tests**、module-recitation **82 tests** + e2e **1 test**、module-wrongbook **21 tests**、suite-core **74 tests**，workspace 合计 **389 tests**；Tauri 应用 **51 tests**（另有客观/主观/M1 独立夹具），两套 Clippy `-D warnings`、Tauri check、前端 build 和 `git diff --check` 全通过。M1.2 隔离 `.app` 已完成 54 条迁移；旧已终审验收库副本升级后仍为 3 条 effect、16 条正式 evidence/outbox/audit、完整性 `ok`、FK=0，且不猜测回填旧 retention。OCR 后只形成建议且不会自动写成绩，M1 机器评分也不会绕过老师终审形成正式证据。真实答题卡/默写/答案文件、真实 provider、跨日期学生纵向录音、阈值、真实老师并行耗时和真实脱敏数据演练仍未验证，正式库未写入。
