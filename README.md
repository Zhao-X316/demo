...
| `crates/module-recitation` M1 域 | ✅ M1.1 + M1.2-3 | 文件名解析、结构化 rubric/评分点、不可变 transcript/AI run、熟练度与评分编排、老师逐点评审、共享总体/逐点/保持学习证据，以及脱敏黄金集只读评估合同 |
...
当前已验证：module-exam **182 tests**、module-knowledge **6 tests**、module-profile **23 tests**、module-recitation **88 tests** + e2e **1 test**、module-wrongbook **21 tests**、suite-core **74 tests**，workspace 合计 **395 tests**；Tauri 应用 **51 tests**（另有客观/主观/M1 独立夹具），两套 Clippy `-D warnings`、Tauri check、前端 build、M1 隔离 `.app` build 和 `git diff --check` 全通过。背诵合成黄金合同覆盖 19 类场景、13 个逐点状态、11 个疑点时间段和 7 个需完整回听样本，固定报告全部匹配且 `production_accuracy_claim_allowed=false`、`release_authorized=false`。M1.2 隔离 `.app` 仍为 54 条迁移；旧已终审验收库副本升级后仍为 3 条 effect、16 条正式 evidence/outbox/audit、完整性 `ok`、FK=0，且不猜测回填旧 retention。真实答题卡/默写/答案文件、真实 provider、跨日期学生纵向录音、真实黄金音频、阈值、真实老师并行耗时和真实脱敏数据演练仍未验证，正式库未写入。
