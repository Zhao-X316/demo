...
> 2026-07-15：M1 A6b 隔离真机 G01-G16 与 M2-A1 手工客观题纵切均已通过。独立分支 `codex/t2-artifacts` 已完成 T2～T6、固定卷入站和首个火山方舟题区适配器；B3a0 已实现拍照文件自然排序、JPEG EXIF/文件时间交叉核对、连续照片版式周期识别、普通试卷/答题卡/默写一次路由确认，以及“起始学号 + 中间缺交 + 固定页数”的老师确认映射。系统会比较后续照片何时重复，自动填写每名学生页数；无法可靠识别时才让老师补一个数字。老师确认顺序后可直接查看按学生归组的缩略图联系表，默认全部清楚，只需点选模糊、反光或缺边页面；一次确认会在同一事务中写质量 revision，并只为完整清楚的学生组建立 attempt 和正式页面匹配。任一页需重拍只扣住该学生整组，后续学生不顺移，且不会生成分数、发布或学习证据。上传照片自动生成机器质量指标/题区/答题格、答题卡 OMR、默写手写 OCR、重拍单点替换、答案资料自动结构化与三类真实黄金集仍未完成；所有范围仍未 tag、合并、推送或发布。
...
| M2 拍照批改 | 🟡 B3a0 有序入站、页型周期、学生页组和质量确认完成·三类识别器待接 | 文件按自然文件名排序并用 EXIF/文件时间核对；本地边缘指纹检测连续页面的最短重复周期并自动填写每人页数，低置信才请老师补录。老师只确认起始学号和缺交学生，再看一次按学生归组的缩略图；模糊页只扣住该学生整组，完整页组原子建立正式归属。普通试卷、答题卡、默写低置信时只问一次；缺页/重复/未交或周期异常不自动顺移。固定普通卷题区 Ark 已接，但整页自动质量/版面/题区、重拍单点替换、答题卡 OMR 与默写 OCR 尚未完成 |
...
当前已验证：module-exam **56 tests**、workspace **185 tests**、Tauri `--all-targets` **33 tests**（外壳 31 + 隔离夹具 2）、module-exam/Tauri Clippy `-D warnings`、Tauri check、前端 `tsc+vite` 和 `git diff --check` 全通过；隔离 T6 夹具的迁移闸门已改为按实际模块迁移数动态核对，当前为 21 个迁移。B3a0 覆盖超长数字自然排序、JPEG EXIF、低置信材料类型、学号自然顺序、起始学号/缺交更正、旧映射保留、重复版式单页/双页周期、PDF 页数建议、真实入站排序/幂等，以及多页学生组中任一页需重拍时整组不建正式归属、后续学生不顺移、相同确认幂等、非法页面整笔回滚。版式周期阈值目前只有合成契约验证，老师质量确认也不等同于机器质量指标；真实学生照片、真实 EXIF 机型覆盖、整页质量/语义页型和三类黄金集尚未验证，不能据此声明识别准确率；上传页 GUI 冒烟仍未完成，正式库未写入。
