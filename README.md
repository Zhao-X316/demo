...
| `crates/module-exam` M2 | 🟡 A1 + B0/B1 + M2.5 + B2 + B3a1/B3a2/B3c3 + M2-C0 | 普通卷、固定答题卡多页模板集/客观主观分流、固定默写首建/复用与正式终审链、五类答案源结构化/冲突预检及客观题、固定填空、固定简答答案另存新版本已落；三材料合成黄金合同、只读评估器、真实材料闸门、受限试点数据权利演练和 hash-only 三材料影子会话已落，真实脱敏样本/provider 仍缺；默写机器结果仍只是建议，简答评分点结构变化仍须老师逐点补录映射 |
...
当前已验证：module-exam **129 tests**、workspace **258 tests**、Tauri 应用 **47 tests**（另有隔离夹具 2 tests）、两套 Clippy `-D warnings`、Tauri test/check、前端 build 和 `git diff --check` 全通过；exam 迁移已到 `exam_0021`。新增回归证明多页答题卡在任一页模板缺失、题型或题区覆盖漂移时 fail-closed；旧纯客观物化记录可确定性回填 `objective_omr` 路由；主观区只进入 `handwriting_ocr` 待处理状态，不产生 OMR 观察、分数、发布或学习证据。既有回归仍证明老师校正默写后须单独终审，发布前不产生正式学习证据；人工补录不会覆盖机器 OCR。真实答题卡/默写/答案文件、真实 provider、阈值、GUI 与真实脱敏数据演练仍未验证，正式库未写入。
