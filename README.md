...
> 2026-07-15：独立分支 `codex/t2-artifacts@09b3182` 已完成 T2～T6、B3a0 有序归组/评分前重拍，以及 B3a1 普通卷结构确认纵切。普通卷质量通过并正式匹配后，系统按页生成不可变 `ordinary_paper_structure` AI run；老师一次确认 ready 结构后，系统在同一事务内创建老师确认的配准、题区、裁剪和不可变确认账本，再进入既有客观题机器建议与老师终审。模型不能新增题目、替老师确认结构、计分、发布或生成学习证据。真实照片/凭据校准、答题卡固定模板与本地 OMR、默写 OCR/评分策略、答案结构化与三类黄金集仍未完成。未 tag、合并、推送或发布。
...
| `crates/module-exam` M2 | 🟡 A1 + B0/B1 + M2.5 + B2 + B3a1 普通卷纵切 | 手工纵切已真机通过；assessment/attempt、页面证据链、私有候选、客观题终审/发布/evidence，以及普通卷 run→老师确认配准/题区/crop→机器建议的审计链已落地 |
...
| M2 页面证据链 / OCR | 🟡 普通卷自动结构确认已接 | 固定夹具覆盖完整页面证据链；Ark 视觉适配器已按 `ordinary_paper_structure run → teacher confirmation → alignment/region/crop → ObjectiveRecognizer → observation` 接入，并拒绝模板外标签、脱敏持久化失败。结果仍须老师逐题终审，真实照片准确率未验证 |
| M2.5 题目自动沉淀 | 🟡 T5 核心合同完成 | 精确复用当前作业已固定的 K1 版本；否则创建老师私有 C0 候选；冲突/歧义进入待确认，学生卷只允许脱敏文本；相似题语义检索、候选整理 UI 和晋级尚未接入 |
| M2 拍照批改 | 🟡 B3a1 普通卷结构确认纵切完成 | 自然排序/时间核验、页数建议、起始学号/缺交、缩略图质量和完整组正式归属已落。模糊页只扣住本学生，可逐页重拍替换；普通卷 ready 结构由老师一次确认后生成配准、题区和裁剪，再进入客观题建议。答题卡固定模板/本地 OMR 与默写 OCR/评分策略尚未完成 |
| M2 傻瓜式固定卷预检 | 🟡 普通卷从上传到机器建议已贯通 | “上传批改”以班级和已确认作业为唯一入口；JPEG/PDF 学生卷及答案候选分级归档，PDF 拆成真实单页 artifact；普通卷自动分析、老师结构确认和客观题识别已串联。答案自动结构化、真实照片 GUI/阈值验收、答题卡和默写专用链仍待施工 |
| 音频回放 | ✅ app-managed archive | 按 hash 归档；原文件改名、重启和数据库恢复后仍可回放，原路径只兜底 |
| ffmpeg 转码/时长探测 | ✅ 可选集成 | 装了 ffmpeg 则转 16k 单声道 wav + 探测时长，否则降级 |

当前已验证：module-exam **65 tests**、workspace **194 tests**、Tauri 应用 **37 tests**、两套 Clippy `-D warnings`、Tauri check、前端 build 和 `git diff --check` 全通过；当前模块迁移共 16 个。新增回归覆盖普通卷 ready 合同、老师确认的原子落库/幂等/越权拒绝，以及“确认结构不产生评分、发布或学习证据”。真实照片、真实 provider 凭据、上传页 GUI、普通卷阈值和三类黄金集尚未验证；正式库未写入。
