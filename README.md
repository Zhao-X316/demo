...
| `crates/module-exam` M2 | 🟡 A1 + B0/B1 + M2.5 + B2 + B3a1/B3a2/B3c3 + M2-C0 | 普通卷、固定答题卡、固定默写首建/复用与正式终审链、五类答案源结构化/冲突预检及客观题、固定填空、固定简答答案另存新版本已落；三材料合成黄金合同、只读评估器、真实材料闸门、受限试点数据权利演练和 hash-only 三材料影子会话已落，真实脱敏样本/provider 仍缺；默写机器结果仍只是建议，简答评分点结构变化仍须老师逐点补录映射 |
...
当前已验证：module-exam **124 tests**、workspace **253 tests**、Tauri 应用 **47 tests**（另有隔离夹具 2 tests）、两套 Clippy `-D warnings`、Tauri test/check、前端 build、三材料影子会话 CLI E2E 和 `git diff --check` 全通过；exam 迁移仍到 `exam_0020`。既有回归证明老师校正默写后仍须单独终审，发布前不产生正式学习证据；人工补录不会覆盖机器 OCR；严格批量只确认当前高置信度精确结果；真实数据评估只有在批准闸门和四类实际 dry-run 回执全部匹配时才能运行。新增影子会话要求普通卷、答题卡和默写一次齐全，合成与真实不得混用；错误识别值、危险批量放行、误接受、缺失/额外输出和覆盖缺口分别形成安全发现，结果固定 `release_authorized=false`。合成 CLI 输出已验证不含 provider 明文、本机路径、学生姓名或答案正文；真实答题卡/默写/答案文件、真实 provider 凭据、阈值、GUI 与真实脱敏数据演练仍未验证，正式库未写入。
