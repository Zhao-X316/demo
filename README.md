...
> 2026-07-15：独立分支 `codex/t2-artifacts@6cb5676` 已完成 T2～T6、B3a0、B3a1 普通卷纵切、B3a2 固定答题卡首建/复用、M2-C0 固定格式默写首条纵切，以及答案图片/TXT/PDF 结构化与冲突预检。答案 PDF 由 macOS CoreGraphics 逐页渲染为 JPEG 后送入同一合同，原 PDF hash、渲染器版本和逐页 hash 均绑定 `ai_run`，模型页码越界会被拒绝。上传答案仍只形成带来源锚点的逐题 AI 草稿；全题与当前作业 K1 答案结构一致时老师一次确认复用，冲突或缺题会阻断并逐题展示，老师可明确沿用当前答案，系统不会静默改写作业答案。普通卷、答题卡和默写的机器观察仍不能自动确认分数、发布或生成正式学习证据。Word/Excel、采纳冲突答案建立 K1/作业新版本、真实照片/provider/GUI 与三类黄金集仍未完成。未 tag、合并、推送或发布。
...
| `crates/module-exam` M2 | 🟡 A1 + B0/B1 + M2.5 + B2 + B3a1/B3a2/B3c1a + M2-C0 纵切 | 普通卷、固定答题卡、固定默写首建/复用链与图片/TXT/PDF 答案逐题结构化/冲突预检已落；默写机器结果仍只是建议，答案冲突尚不能采纳为新 K1/作业版本 |
...
| M2 傻瓜式固定卷预检 | 🟡 三材料固定版式 + 图片/TXT/PDF 答案核对已贯通 | “上传批改”仍是唯一主入口；普通卷确认结构，答题卡确认一次空白模板，默写确认一次空白区域/评分策略，之后整批自动处理并只展示异常。答案全一致一次确认，冲突/缺题阻断且可明确沿用当前答案；Word/Excel 与采纳冲突答案新建版本仍待施工 |
| 音频回放 | ✅ app-managed archive | 按 hash 归档；原文件改名、重启和数据库恢复后仍可回放，原路径只兜底 |
| ffmpeg 转码/时长探测 | ✅ 可选集成 | 装了 ffmpeg 则转 16k 单声道 wav + 探测时长，否则降级 |

当前已验证：module-exam **87 tests**、workspace **216 tests**、Tauri 应用 **44 tests**（另有隔离夹具 2 tests）、两套 Clippy `-D warnings`、Tauri check、前端 build 和 `git diff --check` 全通过；exam 迁移已到 `exam_0018`。新增回归证明答案结构化请求不携带当前标准答案或学生数据、PDF 真正渲染为逐页 JPEG、多页请求携带明确页序且输出页码越界会被拒绝；逐题全一致才允许一次确认，冲突保持阻断且老师明确沿用当前答案后才放行；答案核对不创建成绩、发布或学习证据。真实答题卡/默写/答案照片、真实 provider 凭据、阈值和 GUI 尚未验证；Word/Excel 答案解析、冲突答案新版本向导与默写正式成绩/evidence 尚未完成；正式库未写入。
