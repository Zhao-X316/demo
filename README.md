...
| M2 拍照批改 | 🟡 B3a0 有序入站、重复版式周期识别与学生页组确认完成·三类识别器待接 | 文件按自然文件名排序并用 EXIF/文件时间核对；本地边缘指纹检测连续页面的最短重复周期并自动填写每人页数，低置信才请老师补录。老师只确认起始学号和缺交学生，系统按学号连续建立照片组归属并保留更正 revision。普通试卷、答题卡、默写低置信时只问一次；缺页/重复/未交或周期异常不自动顺移。固定普通卷题区 Ark 已接，但整页自动质量/版面/题区、答题卡 OMR 与默写 OCR 尚未完成 |
...
当前已验证：module-exam **56 tests**、workspace **185 tests**、Tauri `--all-targets` **31 tests**（外壳 29 + 隔离夹具 2）、module-exam/Tauri Clippy `-D warnings`、Tauri check、前端 `tsc+vite` 和 `git diff --check` 全通过；隔离 T6 夹具的迁移闸门已改为按实际模块迁移数动态核对，当前为 20 个迁移。B3a0 覆盖超长数字自然排序、JPEG EXIF、低置信材料类型、学号自然顺序、起始学号/缺交更正、旧映射保留、重复版式单页/双页周期、PDF 页数建议、真实入站排序/幂等，以及确认映射不创建页面匹配或分数。版式周期阈值目前只有合成契约验证，真实学生照片、真实 EXIF 机型覆盖、整页质量/语义页型和三类黄金集尚未验证，不能据此声明识别准确率；上传页 GUI 冒烟仍未完成，正式库未写入。
