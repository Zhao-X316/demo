...
> 2026-07-15：M1 A6b 隔离真机 G01-G16 与 M2-A1 手工客观题纵切均已通过。独立分支 `codex/t2-artifacts` 已完成 T2～T6、固定卷入站和首个火山方舟题区适配器；B3a0 已实现拍照文件自然排序、JPEG EXIF/文件时间交叉核对、普通试卷/答题卡/默写一次路由确认，以及“起始学号 + 中间缺交 + 固定页数”的老师确认映射。顺序、材料类型、页型预览和照片组归属均保留 revision；缺页、重复、页型周期或学生范围不明会扣住自动归属，不把后续全班顺移。该映射仍不会绕过页面质量闸门创建页面匹配或分数。上传照片自动生成质量/题区/答题格、答题卡 OMR、默写手写 OCR、答案资料自动结构化与三类真实黄金集仍未完成；所有范围仍未 tag、合并、推送或发布。
...
| M2 拍照批改 | 🟡 B3a0 有序入站与学生页组确认完成·三类识别器待接 | 文件按自然文件名排序并用 EXIF/文件时间核对；老师只确认起始学号、缺交学生和固定页数，系统按学号连续建立照片组归属并保留更正 revision。普通试卷、答题卡、默写低置信时只问一次；缺页/重复/未交或周期异常不自动顺移。固定普通卷题区 Ark 已接，但整页自动质量/版面/题区、答题卡 OMR 与默写 OCR 尚未完成 |
...
当前已验证：module-exam **53 tests**、workspace **182 tests**、Tauri `--all-targets` **29 tests**（外壳 27 + 隔离夹具 2）、module-exam/Tauri Clippy `-D warnings`、Tauri check、前端 `tsc+vite` 和 `git diff --check` 全通过；隔离 T6 夹具的迁移闸门已改为按实际模块迁移数动态核对，当前为 20 个迁移。B3a0 的 7 条 ordered-intake 单测和 10 条 Tauri 入站测试覆盖超长数字自然排序、低置信材料类型、学号自然顺序、起始学号/缺交更正、旧映射保留、真实入站排序/幂等，以及确认映射不创建页面匹配或分数；JPEG EXIF `DateTimeOriginal` 解析也有合成字节测试。真实学生照片、真实 EXIF 机型覆盖、整页视觉页型分类器和三类黄金集尚未验证，不能据此声明识别准确率；上传页 GUI 冒烟仍未完成，正式库未写入。
