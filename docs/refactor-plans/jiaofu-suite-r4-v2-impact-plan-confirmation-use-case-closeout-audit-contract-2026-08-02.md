# Jiaofu Suite R4-V2 影响计划确认 Use-case 拆分收口审计合同（2026-08-02）

## 1. 审计性质

R4-V2 是 R4-S2 改后状态的独立只读复核。仓库内只允许新增本合同；不得修改父/子模块、测试、Tauri、service/lib、迁移、Cargo、前端或任何业务代码。

本批目标是重新证明：

1. 第二个 use-case 的 243 行主体机械同源；
2. 原公开模块路径和唯一 Tauri caller 未变；
3. transaction owner、SQL、写入顺序、event/audit 和历史禁止写边界未变；
4. E2 late-outbox 失败测试及完整工程矩阵可重复通过；
5. 工作树只因 V2 合同增加一条仓库路径。
...
- 展开 porcelain：177；staged：0
- 父模块：2,088 行，SHA-256 `8eb07a1856590b85552ff37550f0e35ab56c47021fd4668884d569093dab25c5`
- 私有子模块：256 行，SHA-256 `453a10bad5444510588d26f05d3c8f3392a4314e8b18f027b2010b68292a5b99`
- 子模块 243 行主体 SHA-256：`be4b3ca94e5751eb1160f2f504de7b70fdcf82ddc07d1999cb93853e6507f139`
- E2 测试：1,290 行，SHA-256 `31facfb79dcd68526fdd527cde5e451dd8ae1327f225f0f138b4d7e7aafabd73`
...
- S2 验证：专项 11/11、module-exam 210/210、workspace 483/483、Tauri 74/74、两套 Clippy、Tauri check、Vite 95 modules、rustfmt 与 whitespace 全绿。

## 3. 结构与事务复核清单

- 父模块恰好一处 `mod impact_plan_confirmation` 与一处 `pub use impact_plan_confirmation::confirm_question_impact_plan`；
- 公开函数定义只存在于私有子模块；
- 唯一生产 caller 仍只调用父模块原公开路径；
- 子模块主体 hash 与 `/tmp/jiaofu-r4-s2-impact-plan-source-block.rs` 相同；
- 当前父模块与 `/tmp/jiaofu-r4-s2-question-performance.expected.rs` 字节一致；
- 子模块恰好一个 `conn.transaction()`、一个 `tx.commit()`；
- 子模块仍只创建 plan、task、outbox 与 audit；
- 子模块不出现 grade decision、publication、learning evidence 或 profile snapshot/link 的写语句；
- E2 测试仍含指定 impact-plan outbox abort trigger、八类计数回滚、同 request key 重试和历史事实不变断言。
...
全部冻结哈希、HEAD、index 和 staged 必须保持；展开 porcelain 只能因新增本合同 177→178。V2 合同另做 no-index whitespace 检查。

## 5. 完成与边界

V2 全部通过后，只能把“R4 第二个 Rust use-case 拆分闭环”标记完成。整个 R4 的下一动作必须基于只读重新盘点；不得因父文件仍较大就自动选择第三个候选。

V2 不证明老师体验改善、真实 provider、真实数据权利、独立 `.app`、R5、集成、提交、合并或发布。
